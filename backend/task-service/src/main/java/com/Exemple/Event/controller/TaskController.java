...
public class TaskController {
    ...
    @PostMapping("/task")
    public ResponseEntity<TaskResponse> createTask(@Valid @RequestBody TaskRequest request, @AuthenticationPrincipal Jwt jwt){
    ...
    @GetMapping("/tasks")
    public ResponseEntity<List<TaskSummaryResponse>> listTasks(@AuthenticationPrincipal Jwt jwt) {
        return ResponseEntity.ok(eventService.listTasks(ownerId(jwt)));
    }

    @GetMapping("/task/{taskId}")
    public ResponseEntity<TaskDetailResponse> getTask(@PathVariable UUID taskId, @AuthenticationPrincipal Jwt jwt) {
        return ResponseEntity.ok(eventService.getTask(ownerId(jwt), taskId));
    }

    @PostMapping("/{taskId}/blocks")
    public ResponseEntity<BlockResponse> addBlock(@PathVariable UUID taskId, @RequestBody BlockRequest request, @AuthenticationPrincipal Jwt jwt) {
    ...
    @PostMapping("/folders")
    public ResponseEntity<FolderResponse> createFolder(@Valid @RequestBody CreateFolderRequest request, @AuthenticationPrincipal Jwt jwt) {
        return ResponseEntity.status(HttpStatus.CREATED).body(eventService.createFolder(ownerId(jwt), request));
    }

    @GetMapping("/tree")
    public ResponseEntity<TaskTreeResponse> getTree(@AuthenticationPrincipal Jwt jwt) {
    ...
    public ResponseEntity<TaskDocumentResponse> getTaskDocument(@PathVariable UUID taskId, @AuthenticationPrincipal Jwt jwt) {
    ...
    private UUID ownerId(Jwt jwt) {
        if (jwt == null) {
            ...
            throw new org.springframework.web.server.ResponseStatusException(UNAUTHORIZED, "JWT ausente");
        }
        ...
        if (userId == null || userId.isBlank()) {
            ...
            throw new org.springframework.web.server.ResponseStatusException(UNAUTHORIZED, "JWT sem claim userId");
        }
        try {
            return UUID.fromString(userId);
        } catch (IllegalArgumentException ex) {
            throw new org.springframework.web.server.ResponseStatusException(BAD_REQUEST, "JWT com userId inválido");
        }
    }
}
